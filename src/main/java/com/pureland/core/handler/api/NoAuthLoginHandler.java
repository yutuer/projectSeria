...
public class NoAuthLoginHandler extends RequestAPIHandler {
    ...
    public RespWrapper handler(ReqWrapper reqWrapper, String authToken, Long timestamp) throws CoreException {
        ...
        try {
            ...
            LoginResp laLoginResp = loginRespBuilder
                    .setAuthToken(token)
                    .setUserId(userRaceId)
                    .setCurrentTime(System.currentTimeMillis())
                    .setNextItemSid(Building.getIdValueWithUserRace(userRaceId))
                    .build();
            respWrapper = RespWrapperProtocal.RespWrapper.newBuilder()
                    .setLoginResp(laLoginResp)
                    .build();
