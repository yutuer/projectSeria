 ...
 * @author qinpeirong
 */
public class UpgradeUnlockCondition extends BaseCondition implements Conditions {
    ...
    public Boolean validation(BuildingBean validateModel) throws CoreException {
        ...
        if (buildNeedType == SubServerTypeEnum.Crown) {
            ...
            if (hasBuild.getLevel() < entityModel.getUpgradeNeedLevel()) {
                throw new CoreException(String.format("level condition is not enough nowLevel : %d, needLevel : %d", hasBuild.getLevel(), entityModel.getUpgradeNeedLevel()));
            }
        }
        return true;
    }
